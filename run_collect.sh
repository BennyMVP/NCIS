...
# 2. sender in /tmp
echo "[2/6] Copio il sender in /tmp..."
cp scripts/udp_realistic_sender.py /tmp/

# 3. avvia collector in background
echo "[3/6] Avvio collector (background)..."
PYTHONPATH=./ryu python3 ryu/bin/ryu-manager controller/dataset_collector_auto.py \
    > /tmp/collector_auto.log 2>&1 &
RYU_PID=$!
sleep 5   # tempo perche' Ryu si avvii e ascolti su 6653
